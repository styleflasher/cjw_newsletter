...
/**
 * Override of ezComponents SMTP transport class
 * to support "STARTTLS" command
 * ATTENTION: Kernel Overrides don't work for eZComponents
 * so this file needs to be copied to the corresponding folder manually!
 * 
 * 
 * @author René Hrdina
 * @copyright 03/2014 styleflasher.new media og
 */
...
class ezcMailSmtpTransport implements ezcMailTransport
{
    ...
    /**
     * HACK BY RINDA
     * Authenticate with 'STARTTLS', then 'PLAIN'
     */
    
    const AUTH_STARTTLS_PLAIN = "STARTTLS_PLAIN";
    ...
    protected function auth( $method )
    {
        switch ( $method )
        {
        	case self::AUTH_STARTTLS_PLAIN:
        	    $authenticated = $this->authStartTLSPlain();
        	    break;
    ...
    protected function authStartTLSPlain()  {
        
        $this->sendData( 'STARTTLS' );
        $this->getReplyCode( $error );
